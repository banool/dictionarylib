...
void runReviewEarlyExitSuite(DictAppTestConfig cfg) {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      // kSessionCards, both card directions on, and (where the app filters the
      // flashcard pool, e.g. by region) allow everything so no seeded video is
      // filtered out. Start review history from empty so the persisted counts are
      // exact.
      ...
      expect(
        partial.where((r) => r.rating == Rating.Hard).length,
        1,
        reason: 'the one Forgot rating should persist as Hard',
      );
      expect(
        partial.where((r) => r.rating == Rating.Good).length,
        2,
