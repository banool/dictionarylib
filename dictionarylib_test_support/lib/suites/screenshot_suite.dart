...
// The Android capture path needs the raw method channel; the apps have always
// shipped with this implementation import.
...
// each. Run it (and actually save the PNGs) via the app's thin
// integration_test/screenshot_test.dart stub:
//
//   flutter drive \
...
void runScreenshotSuite(DictAppTestConfig cfg, ScreenshotSuiteConfig shots) {
  ...
  testWidgets("takeScreenshots", (WidgetTester tester) async {
    ...
    // Apps that load the dictionary over the network usually have it ready by
    // here, but wait defensively so the seeding below isn't a no-op on a slow
    // fetch. Exits immediately when the data ships bundled.
    ...
    for (final word in shots.animalsSeedWords) {
      final entry = keyedByEnglishEntriesGlobal[word];
      if (entry != null) await animalList.addAllVideosOfEntry(entry);
    }

    // Revise the Animals list with spaced repetition, seeding any app-specific
    ...
    Future<void> capture(String name) =>
        takeScreenshot(tester, binding, info, shots.localeDirName, name);
    ...
    await settle(tester);
    await tester.enterText(searchField, shots.searchQuery);
    await capture("searchResults");
    ...
    final heroEntry = keyedByEnglishEntriesGlobal[shots.heroEntryKey]!;
    cfg.navigateToEntryPage(rootNavigatorKey.currentContext!, heroEntry, true);
    await letVideoLoad(tester);
    await capture("wordPage");
    ...
    if (rotationIsNoOp) {
      ...
      if (!Platform.isAndroid) {
        ...
        await tester.tap(find.byKey(const ValueKey("startButton")));
        await letVideoLoad(tester);
        await capture("flashcardFrontLandscape");

        // 14. The same flashcard revealed: video left, word + rating buttons
        // right.
        await tester.tap(find.byKey(const ValueKey("revealButton")));
        await letVideoLoad(tester);
        await capture("flashcardRevealedLandscape");
        await tester.tap(find.widgetWithIcon(IconButton, Icons.close));
        await settle(tester);
      }
