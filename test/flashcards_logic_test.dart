...
void main() {
  ...
  group('migrateLegacyReviewsIfNeeded — short-circuits', () {
    test('no stored reviews + no flag → sets flag, writes nothing', () async {
      ...
      expect(
        sharedPreferences.getInt(KEY_REVIEWS_SCHEMA_VERSION),
  ...
  group('migrateLegacyReviewsIfNeeded — v1 master rewrite', () {
    test('legacy master with tail that endsWith a known video URL '
      ...
      keyedByEnglishEntriesGlobal['apple'] = FakeEntry(
        'apple',
        videos: const ['https://media.test/apple.mp4'],
      );
    ...
    test('legacy master with tail that "contains" a video URL '
      ...
      keyedByEnglishEntriesGlobal['friend'] = FakeEntry(
        'friend',
    ...
    test('fallback picks the first sub-entry\'s first video when the '
      ...
      keyedByEnglishEntriesGlobal['multi'] = FakeEntry(
        'multi',
        subEntries: const [
    ...
    test('hyphenated entry key resolves via the right split point', () async {
      ...
      keyedByEnglishEntriesGlobal['welsh-corgi'] = FakeEntry(
        'welsh-corgi',
    ...
    test('longest matching entry key wins when a shorter prefix is also '
      ...
      // wrong entry forever.
      keyedByEnglishEntriesGlobal['welsh'] = FakeEntry(
        'welsh',
  ...
  group('migrateLegacyReviewsIfNeeded — v2 inputs + mixed inputs', () {
    test(
      'a v2 master whose URL is under no known base is kept verbatim',
      () async {
        ...
        keyedByEnglishEntriesGlobal['apple'] = FakeEntry(
          'apple',
          videos: const ['https://media.test/a.mp4'],
        );
    ...
    test('mixed input: v1 + v2 in same list — v1 migrated, v2 untouched, '
      ...
      keyedByEnglishEntriesGlobal['banana'] = FakeEntry(
        'banana',
        videos: const ['https://media.test/b.mp4'],
      );
    ...
    test('mixed input with one unresolvable v1 — bad one dropped, '
      ...
      keyedByEnglishEntriesGlobal['cherry'] = FakeEntry(
        'cherry',
        videos: const ['https://media.test/c.mp4'],
      );
    ...
    test(
      'malformed encoded review (no delimiter) is dropped silently',
      () async {
  ...
  group('migrateLegacyReviewsIfNeeded — semantic preservation', () {
    test('a v1 review\'s ts / rating / combination round-trip through '
      ...
      final encoded = encodeFakeReview(
        'apple-a.mp4',
        front: 1,
        back: 0,
        rating: Rating.Hard,
    ...
    test(
      'decoded v2 master matches what savedVideoMasterId would produce',
      () async {
  ...
  group('migrateLegacyReviewsIfNeeded — v2→v3 path conversion', () {
    ...
    test(
      'a v2 master URL under the configured base is rewritten to a path',
      () async {
  ...
  group('migrateLegacyReviewsIfNeeded — SLSL suffix-shaped masters', () {
    test(
      '"<filename><entryKey>" resolves to the media path by filename',
      () async {
        keyedByEnglishEntriesGlobal['Sri Lanka'] = FakeEntry(
          'Sri Lanka',
          videos: const ['/media/11450.mp4'],
        );
        await sharedPreferences.setStringList(KEY_STORED_REVIEWS, [
    ...
    test(
      'a multi-word entry key with a space resolves via the suffix match',
      () async {
        keyedByEnglishEntriesGlobal['good morning'] = FakeEntry(
          'good morning',
          videos: const ['/media/77.mp4'],
        );
        await sharedPreferences.setStringList(KEY_STORED_REVIEWS, [
    ...
    test(
      'the longest entry-key suffix wins ("Sri Lanka" over "Lanka")',
      () async {
        // Both exist and end the master; a naive shortest/any match would
        // mis-attribute the review to "Lanka" forever.
        keyedByEnglishEntriesGlobal['Lanka'] = FakeEntry(
          'Lanka',
          videos: const ['/media/lanka.mp4'],
        );
        keyedByEnglishEntriesGlobal['Sri Lanka'] = FakeEntry(
          'Sri Lanka',
          videos: const ['/media/11450.mp4'],
    ...
    test(
      'an old full-URL video form still resolves by trailing filename',
      () async {
    ...
    test(
      'entry matched but the filename is unknown → first-video fallback',
      () async {
        keyedByEnglishEntriesGlobal['Sri Lanka'] = FakeEntry(
          'Sri Lanka',
          videos: const ['/media/real.mp4'],
        );
        await sharedPreferences.setStringList(KEY_STORED_REVIEWS, [
    ...
    test(
      'a master whose entry is gone is dropped (others survive, no crash)',
      () async {
        keyedByEnglishEntriesGlobal['Sri Lanka'] = FakeEntry(
          'Sri Lanka',
          videos: const ['/media/11450.mp4'],
    ...
    test('a filename whose pre-hyphen prefix is also an entry resolves to the '
      ...
      keyedByEnglishEntriesGlobal['cold'] = FakeEntry(
        'cold',
        videos: const ['/media/cold.mp4'],
      );
      keyedByEnglishEntriesGlobal['snobbish'] = FakeEntry(
        'snobbish',
    ...
    test(
      'the resolved master equals savedVideoMasterId for the same video',
      () async {
