...
void main() {
  ...
  group('EntryList.getKeyFromName / getNameFromKey', () {
    ...
    test('rejects names longer than maxListNameLength', () {
      expect(
        ...
        throwsA(
          isA<EntryListNameException>().having(
            (e) => e.kind,
            'kind',
            EntryListNameError.tooLong,
          ),
        ),
      );
      // Boundary: exactly the max length is fine.
      expect(
        () => EntryList.getKeyFromName('a' * maxListNameLength),
        returnsNormally,
      );
    });
    ...
    test(
      'tolerates underscores by default, displaying them as spaces (lossy)',
      () {
      ...
      expect(
        ...
        throwsA(
          isA<EntryListNameException>().having(
            (e) => e.kind,
            'kind',
            EntryListNameError.underscore,
          ),
        ),
      );
      // A space is fine; only the literal underscore is refused.
      expect(
        () => EntryList.getKeyFromName('a b', rejectUnderscores: true),
        returnsNormally,
      );
    });
  });
  ...
  group('EntryList persistence', () {
    test(
      'addVideo persists the saved video (by path) to sharedPreferences',
      () async {
  ...
  group('EntryList migration → v3', () {
    ...
    test(
      'a v2 list with no version flag is treated as v1 and stripped',
      () async {
    ...
    test(
      'order is preserved: sub-entries first, then within-sub-entry videos',
      () async {
    ...
    test(
      'legacy entry with zero videos is dropped, not added as empty',
      () async {
        keyedByEnglishEntriesGlobal['voiceless'] = FakeEntry('voiceless');
        await sharedPreferences.setStringList('cats_words', [
          'voiceless',
    ...
    test(
      'duplicate legacy entries collapse to one set of saved videos',
      () async {
        await sharedPreferences.setStringList('cats_words', [
          'apple',
          'apple',
    ...
    test(
      'a list where every legacy entry got dropped still sets the flag',
      () async {
        await sharedPreferences.setStringList('cats_words', [
          'no_such_entry_1',
          'no_such_entry_2',
    ...
    test('migrated list answers the new read API correctly', () async {
      ...
      expect(list.containsEntry(keyedByEnglishEntriesGlobal['apple']!), isTrue);
      expect(
        list.videosForEntry(keyedByEnglishEntriesGlobal['apple']!),
        hasLength(2),
      );
    ...
    test(
      'a media URL not under any known base is kept as-is, not dropped',
      () async {
    ...
    test(
      ...
      'pipe round-trips through migration',
      () async {
    ...
    test(
      'in-memory state is correct regardless of the fire-and-forget write',
      () async {
        seedDictionary(['apple']);
        await sharedPreferences.setStringList('cats_words', ['apple']);
        final first = EntryList.fromRaw('cats_words');
        ...
        expect(
          reloaded.savedVideos.map((v) => v.toStorage()),
          first.savedVideos.map((v) => v.toStorage()),
        );
      },
    );

    test('UserEntryListManager.fromStartup triggers migration on every '
        'managed list', () async {
      seedDictionary(['apple', 'banana']);
      await sharedPreferences.setStringList(KEY_ENTRY_LIST_KEYS, [
        'favourites_words',
        'cats_words',
      ]);
      ...
      expect(
        sharedPreferences.getInt('favourites_words_schemaVersion'),
        listSchemaVersion,
      );
      expect(
        sharedPreferences.getInt('cats_words_schemaVersion'),
  ...
  group('UserEntryListManager', () {
    ...
    test('createEntryList rejects duplicate keys', () async {
      ...
      await userEntryListManager.createEntryList('cats_words');
      expect(
        () => userEntryListManager.createEntryList('cats_words'),
        throwsA(anything),
      );
    });
    ...
    test('reorder refuses to move favourites away from index 0', () async {
      ...
      expect(
        userEntryListManager.getEntryLists().keys.first,
        KEY_FAVOURITES_ENTRIES,
      );
    });
    ...
    test(
      'renameEntryList moves the list + its videos, keeping position',
      () async {
        userEntryListManager = UserEntryListManager.fromStartup();
        await userEntryListManager.createEntryList('a_words');
        await userEntryListManager.createEntryList('b_words');
    ...
    test('renameEntryList rejects a name that already exists', () async {
      ...
      await userEntryListManager.createEntryList('b_words');
      expect(
        () => userEntryListManager.renameEntryList('a_words', 'b_words'),
        throwsA(isA<EntryListNameException>()),
      );
      expect(
        userEntryListManager.getEntryLists().containsKey('a_words'),
        isTrue,
      );
    });
    ...
    test('renameEntryList refuses to rename favourites', () async {
      ...
      expect(
        sharedPreferences.getStringList(KEY_ENTRY_LIST_KEYS)?.first,
        KEY_FAVOURITES_ENTRIES,
      );
    });
