...
app's screenshots/upload_screenshots.py is a thin wrapper that calls
configure(...) with its app-specific values (bundle ids, locale maps, image
slot maps) and then main(). Run the wrapper, not this file.
...
PLAY_LOCALE_MAP = None       # capture locale dir -> Play listing language
IOS_LOCALE_MAP = None        # capture locale dir -> App Store listing locale
ANDROID_IMAGE_TYPES = None   # capture WxH -> [Play image slots]


def configure(*, project_root, ios_bundle_id, android_package_name,
              play_locale_map, ios_locale_map, android_image_types):
    """Set the app-specific values. Must be called before main().

    play_locale_map / ios_locale_map: the capture directories are named after
    the app locale used at capture time, but the store listings may use a
    different language code (e.g. auslan captures en-AU but its Play listing
    has only ever had en-US) — these map the capture locale onto each store's.
    Unmapped locales pass through unchanged.

    android_image_types: Google Play has one image slot per device class. The
    WxH token take_screenshots.py embeds in each filename identifies the
    capture device, and this maps it onto the Play slots to fill (a tablet
    capture typically fills both tablet slots; an empty list keeps a capture
    local-only, e.g. auslan's touch-TV format that has no Play slot).
    """
    global PROJECT_ROOT, SCREENSHOTS_DIR, IOS_PUBLISH_ENV, PLAY_KEY_PATH
    global IOS_BUNDLE_ID, ANDROID_PACKAGE_NAME
    ...
    PLAY_LOCALE_MAP = dict(play_locale_map)
    IOS_LOCALE_MAP = dict(ios_locale_map)
    ANDROID_IMAGE_TYPES = dict(android_image_types)
...
# their filename; the slots accept either orientation. (Shared: both apps
# capture on the same iOS device matrix.)
IOS_DISPLAY_TYPES = {
...
def upload_ios(plan):
    ...
    touched_sets = {}
    for locale, groups in plan.items():
        asc_locale = IOS_LOCALE_MAP.get(locale, locale)
        loc = locs_by_locale.get(asc_locale)
        if loc is None:
            raise RuntimeError(
                f"the App Store listing has no {asc_locale} localization (has: "
...
if __name__ == "__main__":
    raise SystemExit(
        ...
        "screenshots/upload_screenshots.py wrapper instead."
    )
