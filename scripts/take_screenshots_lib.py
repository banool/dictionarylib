...
app's screenshots/take_screenshots.py is a thin wrapper that calls
configure(...) with its app-specific values (locale dir, device targets,
poster video sources) and then main(). Run the wrapper, not this file.

The simulators and emulators this needs are CREATED ON DEMAND (and reused on
later runs), so you don't have to set any up by hand first — see the
ios_targets / android_targets the wrapper passes. Captures land in
...
# --- App-specific values, set by the app wrapper via configure(). ---

PROJECT_ROOT = None      # Path: the Flutter app dir (holds screenshots/)
SCREENSHOTS_DIR = None   # Path: PROJECT_ROOT / "screenshots"
POSTERS_DIR = None       # Path: SCREENSHOTS_DIR / ".posters"
LOCALE_DIR = None        # str: capture locale dir, e.g. "en-AU" / "en"
IOS_TARGETS = None       # [(simulator name, simctl devicetype id)]
ANDROID_TARGETS = None   # [(AVD name, avdmanager device profile id)]
POSTER_VIDEO_URLS = None    # () -> [video url] for the seeded words
POST_CREATE_AVDS = None     # optional () -> None hook after AVD creation


def configure(*, project_root, locale_dir, ios_targets, android_targets,
              poster_video_urls, post_create_avds=None):
    """Set the app-specific values. Must be called before main().

    ios_targets: App Store Connect requires a 6.9" iPhone and (for iPad apps)
    a 13" iPad, and scales those down for every smaller device. Each entry is
    (simulator name — which shows up in the screenshot path — and the
    `xcrun simctl list devicetypes` identifier to create it from). The newest
    installed iOS runtime is used automatically.

    android_targets: Google Play requires phone screenshots and, for tablet
    support, 7"/10" tablet screenshots. Each entry is (AVD name,
    `avdmanager list device` profile id).

    poster_video_urls: returns the video URLs for the words
    screenshot_test.dart seeds (see the posters comment below); the data
    source is app-specific (bundled data file vs runtime dump).

    post_create_avds: optional hook run after AVD creation for app-specific
    AVD surgery (e.g. auslan's touch-TV config.ini resize).
    """
    global PROJECT_ROOT, SCREENSHOTS_DIR, POSTERS_DIR, LOCALE_DIR
    global IOS_TARGETS, ANDROID_TARGETS, POSTER_VIDEO_URLS, POST_CREATE_AVDS
    PROJECT_ROOT = Path(project_root)
    SCREENSHOTS_DIR = PROJECT_ROOT / "screenshots"
    POSTERS_DIR = SCREENSHOTS_DIR / ".posters"
    LOCALE_DIR = locale_dir
    IOS_TARGETS = list(ios_targets)
    ANDROID_TARGETS = list(android_targets)
    POSTER_VIDEO_URLS = poster_video_urls
    POST_CREATE_AVDS = post_create_avds
...
def ensure_posters():
    ...
    cached *.png after changing extraction settings). The sources are small
    ...
    bilinear filter. The URL list comes from the app wrapper: the data source
    (bundled data file vs runtime dump) is app-specific."""
    urls = POSTER_VIDEO_URLS()
...
def ensure_android_avds():
    ...
    # App-specific AVD surgery (e.g. auslan's touch-TV config.ini resize),
    # idempotent by contract: re-applied every run so a hand-edited or stale
    # config converges back.
    if POST_CREATE_AVDS is not None:
        POST_CREATE_AVDS()
...
def main():
    if PROJECT_ROOT is None:
        raise RuntimeError("configure(...) must be called before main()")
    args = parse_args()
    logging.basicConfig(
    ...
    if args.clear_screenshots:
        for platform in ("ios", "android"):
            leaf = SCREENSHOTS_DIR / platform / LOCALE_DIR
            if leaf.exists():
                shutil.rmtree(leaf)
...
if __name__ == "__main__":
    raise SystemExit(
        "This is the shared library — run the app's "
        "screenshots/take_screenshots.py wrapper instead."
    )
